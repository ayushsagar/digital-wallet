...
1. A matrix representation would waste memory space because the graph is sparsely connected.
...
The time complexity is O(*b*^*d*) and space-complexity is O(*b***d*). *b* is the branching factor. The average branching factor would be the average no. of users each user is connected to. Therefore *b* is expected to be low. Depth *d* is 4 in this case. Therefore the complexity turns out to be a small no and this appears to suitable for the problem.

The same function is used to implement feature 1 and 2 with d = 1 and 2 respectively.
